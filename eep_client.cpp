...
void EEPClient::FnEEPClientInit(const std::string& serverIP, unsigned short serverPort, int eepSourceId, int eepDestinationId, int eepCarparkID)
{
    client_ = std::make_unique<AppTcpClient>(ioContext_, serverIP, serverPort);
    serverIP_ = serverIP;
    serverPort_ = serverPort;
    eepSourceId_ = eepSourceId;
    eepDestinationId_ = eepDestinationId;
    eepCarparkID_ = eepCarparkID;
...
void EEPClient::FnSendAck(uint8_t reqDataTypeCode_)
{
...
void EEPClient::FnSendNak(uint8_t reqDataTypeCode_, uint8_t reasonCode_)
{
    // Data Format in little-Endian
...
{
    enqueueCommand(CommandType::START_REQ_CMD, static_cast<int>(PRIORITY::NORMAL), nullptr);
}
...
void EEPClient::FnSendDeductReq(const std::string& obuLabel_, const std::string& fee_, const std::string& entryTime_, const std::string& exitTime_)
{
    ...
    bool obuParseSuccess = Common::getInstance()->FnConvertDecimalStringToByteArray(obuLabel_, obuLabelArr, 5, true);
    uint8_t feeArr[2];
...
void EEPClient::FnSendCPOInfoDisplayReq(const std::string& obuLabel_, const std::string& dataType_, const std::string& line1_, const std::string& line2_, const std::string& line3_, const std::string& line4_, const std::string& line5_)
{
    ...
    if (obuParseSuccess && dataTypeParseSuccess)
    {
        ...
        std::strncpy(reqData->dataString1, line1_.c_str(), sizeof(reqData->dataString1));
        reqData->dataString1[sizeof(reqData->dataString1) - 1] = '\0';  // Ensure null-termination
        std::strncpy(reqData->dataString2, line2_.c_str(), sizeof(reqData->dataString2));
        reqData->dataString2[sizeof(reqData->dataString2) - 1] = '\0';  // Ensure null-termination
        std::strncpy(reqData->dataString3, line3_.c_str(), sizeof(reqData->dataString3));
        reqData->dataString3[sizeof(reqData->dataString3) - 1] = '\0';  // Ensure null-termination
        std::strncpy(reqData->dataString4, line4_.c_str(), sizeof(reqData->dataString4));
        reqData->dataString4[sizeof(reqData->dataString4) - 1] = '\0';  // Ensure null-termination
        std::strncpy(reqData->dataString5, line5_.c_str(), sizeof(reqData->dataString5));
        reqData->dataString5[sizeof(reqData->dataString5) - 1] = '\0';  // Ensure null-termination
...
void EEPClient::printField(std::ostringstream& oss, const std::string& label, uint64_t value, int hexWidth, const std::string& remark = "")
{
    ...
    oss << std::setw(32) << std::setfill(' ') << ""                    // indent
        << std::left << std::setw(40) << label                         // label left-aligned
        << ": "
        << std::right << std::setw(10) << hexStream.str()              // hex right-aligned
        << " "
        << std::right << std::setw(12) << decStream.str();             // dec right-aligned
    ...
    if (!body.empty() && (body.size() == header.dataLen_))
    {
        ...
        switch (code)
        {
            ...
            case MESSAGE_CODE::OBU_INFORMATION_NOTIFICATION:
            {
                ...
                uint64_t obulabel = Common::getInstance()->FnReadUint40LE(body, 8);
                ...
                std::reverse(vechicleNumber.begin(), vechicleNumber.end());
                uint32_t rsv3 = Common::getInstance()->FnReadUint24LE(body, 34);
                uint8_t cardValidity = body[37];
                uint32_t rsv4 = Common::getInstance()->FnReadUint24LE(body, 38);
                std::vector<uint8_t> can(body.begin() + 41, body.begin() + 49);
                std::reverse(can.begin(), can.end());
                uint32_t cardBalance = Common::getInstance()->FnReadUint32LE(body, 49);
                uint8_t backendAccount = body[53];
                uint8_t backendSetting = body[54];
                uint8_t businessFunctionStatus = body[55];
                uint8_t rsv5 = body[56];
            ...
            case MESSAGE_CODE::TRANSACTION_DATA:
            {
                ...
                uint64_t obuLabel = Common::getInstance()->FnReadUint40LE(body, 16);
                ...
                std::reverse(bepTimeOfReport.begin(), bepTimeOfReport.end());
            ...
            case MESSAGE_CODE::CPO_INFORMATION_DISPLAY_RESULT:
            {
                ...
                uint64_t obuLabel = Common::getInstance()->FnReadUint40LE(body, 12);
            ...
            case MESSAGE_CODE::DSRC_STATUS_RESPONSE:
            {
                ...
                uint64_t OBU1_obulabel = Common::getInstance()->FnReadUint40LE(body, 12);
                ...
                uint64_t OBU2_obulabel = Common::getInstance()->FnReadUint40LE(body, 36);
                ...
                uint64_t OBU3_obulabel = Common::getInstance()->FnReadUint40LE(body, 60);
                ...
                uint64_t OBU4_obulabel = Common::getInstance()->FnReadUint40LE(body, 84);
...
void EEPClient::handleParsedMessage(const MessageHeader& header, const std::vector<uint8_t>& body)
{
    std::ostringstream oss;
    MESSAGE_CODE code = static_cast<MESSAGE_CODE>(header.dataTypeCode_);
        ...
        if (isValidCheckSum(data))
        {
            ...
            if (parseMessage(dataWithoutCheckSum, msgHeader, msgBody))
            {
                if (isValidSourceDestination(msgHeader.sourceID_, msgHeader.destinationID_))
                {
                    ...
                    handleParsedMessage(msgHeader, msgBody);
                }
                else
                {
    ...
    boost::asio::post(strand_, [this, type, priority, data] () {
        {
            ...
            commandQueue_.emplace(type, priority, data);
        }
        checkCommandQueue();
    });
}
    ...
    if (!commandQueue_.empty())
    {
        ...
        while (!tempQueue.empty())
        {
            ...
            oss << "[Cmd: " << getCommandString(cmdData.type)
                << " , Priority: " << cmdData.priority << "]" << std::endl;
            tempQueue.pop();
        }
    }
    else
...
std::vector<uint8_t> EEPClient::prepareCmd(Command cmd)
{
    ...
    switch (cmd.type)
    {
        ...
        {
            // Message Header
            appendMessageHeader(msg, static_cast<uint8_t>(MESSAGE_CODE::ACK), seqNo, 0x0004);
            
            // Message Content
            if (cmd.data)
            {
                msg.insert(msg.end(), cmd.data->serialize().begin(), cmd.data->serialize().end());
            }
            break;
        }
        case CommandType::NAK:
        {
            // Message Header
            appendMessageHeader(msg, static_cast<uint8_t>(MESSAGE_CODE::NAK), seqNo, 0x0004);
        ...
        {
            // Message Header
            appendMessageHeader(msg, static_cast<uint8_t>(MESSAGE_CODE::CPO_INFORMATION_DISPLAY_REQUEST), seqNo, 0x0095);
...
{
    reconnectTimer_.expires_from_now(boost::posix_time::seconds(3));
...
{
    connectTimer_.expires_from_now(boost::posix_time::seconds(5));
...
{
    sendTimer_.expires_from_now(boost::posix_time::seconds(2));
...
{
    responseTimer_.expires_from_now(boost::posix_time::seconds(2));
...
void EEPClient::startWatchdogTimer()
{
    ...
    FnSendWatchdogReq();
    watchdogTimer_.expires_from_now(boost::posix_time::seconds(10));
...
void EEPClient::startHealthStatusTimer()
{
    ...
    healthStatusTimer_.expires_from_now(boost::posix_time::seconds(60));
